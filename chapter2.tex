...
\begin{karte}{Definition Relation}
    Eine \textit{Relation} auf einer Menge \(X\)  ist eine Teilmenge 
    von \( X \times X \).
\end{karte}
\begin{karte}{Definition Äquivalenzrelation}
    Eine \textit{Äquivalenzrelation} ist:
    \begin{description}
...
\begin{karte}{Definition obere Schranke}
    Ein Element \( s \in X \) ist eine \textit{obere Schranke} für 
...
\begin{karte}{Definition Abbildung}
    Eine \textit{Abbildung} \(f : X \rightarrow Y\) ist eine Vorschrift, 
...
\begin{karte}{Definition Innere Verknüpfung}
    ...
    ist eine \textit{innere Verknüpfung} auf der Menge \(M\).
\end{karte}
\begin{karte}{Defintion Gruppe}
    Eine \textit{Gruppe} \( (G,*) \) besteht aus einer Menge 
    ...
    a * b = b * a \), so ist \( (G,*) \) \textit{abelsch} 
    oder \textit{kommutativ}. \\
...
\begin{karte}{Definition Körper}
    Ein \textit{Körper} \( (K,+,\cdot) \) besteht aus einer Menge 
    \(K\) und \\ 
...
\begin{karte}{Null- und Einselement im Körper}
    Das neutrale Element der Addition wird \textit{Nullelement} genannt und 
    das neutrale Element der Multiplikation wird \textit{Einselement} genannt.
\end{karte}
\begin{karte}{Definition Teilkörper}
    Für \( L \subset K \) und \( (K,+,\cdot) \) ist \(L\) 
    ein \textit{Teilkörper}, wenn sich die Addition \(+\) und die 
...
\begin{karte}{Definition Norm und komplexe Konjugation}
    Das \textit{Norm} einer Zahl \( z=x+iy \in \C \) lautet 
    \( \abs{z} = \sqrt{x^2+y^2} \) und das \textit{komplex 
